...
Author: Igor Korobitsyn, naniduan@yandex.ru
...
- Interpreter
- Inferencer
- Labeled and Optional arguments (embeded into type system)
- Demos

Syntactic sugar is not supported (for example, `if-then-else`, defining functions using `let` without `fun`, passing optional arguments using `~arg:value` syntax instead of `?arg:(Some value)`).

Note: erasure of optional arguments is not considered syntactic sugar and is supported.
